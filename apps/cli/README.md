...
CLI for [Open Visual Regression](https://github.com/open-visual-regression/open-visual-regression). Uploads a Storybook build, and lists and inspects builds afterward.
...
| Personal access token | Your account's access tokens settings page | every `ovr builds`, `ovr snapshots` and `ovr diffs` command |
...
- `ovr diffs` inspects a snapshot's diff against its baseline — full reference at [docs.openvisualregression.com](https://docs.openvisualregression.com/cli/diffs)
