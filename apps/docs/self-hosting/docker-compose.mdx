...
| `OVR_DIFF_CONCURRENCY` | `4` | Snapshots compared against their baseline in parallel per worker. Memory scales with this: a comparison decodes the capture and its baseline and builds the diff image, all uncompressed. |
