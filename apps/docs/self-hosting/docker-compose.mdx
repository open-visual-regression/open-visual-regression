...
- 2 vCPU / 4 GB RAM handles a few builds a day. 4 vCPU / 8 GB handles dozens. Past that, add workers: `docker compose up -d --scale worker=3`.
- Storage sized against stories × viewports × builds. Every snapshot is a full-page PNG. Postgres and Redis stay small. Builds are purged after 90 days by default.
...
| Object storage | `rustfs`, `createbuckets` | `STORAGE_BUCKET`, `STORAGE_REGION`, `STORAGE_ACCESS_KEY`, `STORAGE_SECRET_KEY`, plus `STORAGE_ENDPOINT` / `STORAGE_PUBLIC_ENDPOINT` / `STORAGE_FORCE_PATH_STYLE` for anything other than AWS S3 |

Also drop the removed service from `depends_on` on `migrate`, `web`, and `worker`, and drop its volume. Create the bucket yourself first.
...
State lives in named volumes: `pgdata`, `valkeydata`, `rustfsdata`. Back them up like any container volume. There is no separate export step.
