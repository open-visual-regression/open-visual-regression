...
| `REDIS_MODE` | `standalone` (default) or `cluster`. Set `cluster` for a Redis Cluster, with `REDIS_URL` pointing at its configuration endpoint. |

The queue requires `maxmemory-policy noeviction`. Managed Redis services often default to an eviction policy, which can silently drop queued jobs.
...
| `OVR_DIFF_CONCURRENCY` | `4` | Snapshots compared against their baseline in parallel per worker. |
| `OVR_REAPER_STALE_MINUTES` | `30` | Minutes before a stalled capture job is requeued. |
| `OVR_STORYBOOK_CACHE_BYTES` | `2147483648` (2 GB) | Local disk for extracted Storybook builds. |
