...
CPU and memory scale with concurrent captures, not total build volume. Each capture group in flight runs its own browser instance, and each diff in flight holds two decoded images in memory.
...
| `worker.diffConcurrency` | app default `4` | Snapshots compared against their baseline at once, each holding two images and a diff mask in memory. |
