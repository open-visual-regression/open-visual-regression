...
CPU and memory scale with concurrent captures, not total build volume. Each capture group in flight runs its own browser instance and keeps its build's Storybook bundle on disk while sibling groups are still running.
...
| `worker.storybookCacheBytes` | app default 2 GB | Pod-local disk for extracted Storybook builds, reused across a build's capture groups. Counts against the pod's ephemeral storage. |
