...
- [ ] 1.7 `packages/services/src/projects.ts` — add `addVariant(projectId, data, callerId)` + `removeVariant(variantId, callerId)`:
  - `addVariant`: `db.variants.countByProject(projectId)` → throw `LimitExceededError` if ≥ 10; `db.variants.addVariant(...)`
  - `removeVariant`: `db.variants.deleteVariant(variantId)`
