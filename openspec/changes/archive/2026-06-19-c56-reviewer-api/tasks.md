...
- [x] 1.2 `packages/api/src/contracts/projects.ts`: (`buildStatusSchema` already got `"rejected"` in c53-build-finalize-rejected)
...
- [x] 1.3 `packages/db/src/repository/projects.ts`: confirmed in c52-reviewer-schema (1.7) — no further change here, just verify `getProject`/`listProjects` already return `requiredReviewerCount`

- [x] 1.4 `packages/api/src/contracts/diffs.ts` (new):
  ```ts
...
- [x] 1.5 `packages/api/src/contracts/contract.ts`: register `diffs: { ...diffsContract }`

- [x] 1.6 `apps/web/lib/router/diffs.ts` (new): `"use server"`; each handler `.use(authenticatedMiddleware)` + `.actionable()`:
...
- [x] 1.7 `apps/web/lib/router/index.ts`: register `diffs` on `serverClient`

- [x] 1.8 `apps/web/lib/router/builds.ts`: update `getSnapshotDisplayStatus` to read the split columns:
...
- [x] 1.9 Integration tests (`apps/web/lib/router/__tests__/diffs.integration.test.ts`):
