...
- [x] 1.1 Create `apps/web/app/(authenticated)/projects/[projectId]/builds/[buildId]/page.tsx` (RSC):
  - Fetch build + all diffs + snapshots
  - Accept `searchParams: { filter?: "changed" | "pass" }` — default "all"

- [x] 1.2 Build `RunHeader` in `apps/web/lib/components/run-header/`:
...
- [x] 1.3 Build `SnapshotGrid` in `apps/web/lib/components/snapshot-grid/`:
...
- [x] 1.4 Build `SnapshotCard` in `apps/web/lib/components/snapshot-grid/`:
