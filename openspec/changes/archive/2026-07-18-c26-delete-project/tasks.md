...
- [x] 1.1 `packages/queue/src/index.ts` — add `QueueName.PROJECT_PURGE = "project-purge"`;
...
- [x] 1.2 `packages/queue/src/producer.ts` — export the connection-bound
  `enqueueProjectPurge(payload)`.

## Worker — purge handler (apps/worker)

- [x] 2.1 `apps/worker/src/handlers/projectPurge.ts` — `run(job)` calls
  `storage.deletePrefix(\`${job.data.projectId}/\`)`; `failed(job, error)` logs.
- [x] 2.2 `apps/worker/src/index.ts` — register a `Worker(QueueName.PROJECT_PURGE,
  projectPurge.run, { connection })` and wire `.on("failed", guard(projectPurge.failed))`.
- [x] 2.3 Handler test: `run` calls `storage.deletePrefix` with exactly
  `${projectId}/` (the prefix is the job's whole contract).

## DB repository (packages/db)

- [x] 3.1 `packages/db/src/repository/projects.ts` — change
...
- [x] 4.1 `packages/api/src/contracts/projects.ts` — add `deleteProjectContract`
  (input `{ id: z.uuidv7() }`; output `z.void()`) and register it on the
  `contract` object.

## Router handler (apps/web)

- [x] 5.1 `apps/web/lib/router/projects.ts` — add `deleteProject`:
...
- [x] 5.2 Handler integration tests
...
- [x] 6.1 `settings/_components/delete-project/DeleteProjectSection.tsx` — the
...
- [x] 6.2 `settings/page.tsx` — render `<DeleteProjectSection project={…}>` at the
  bottom (no extra data fetch). Already admin-gated by the page's `verifyRole`.
- [x] 6.3 `settings/_components/delete-project/DeleteProjectDialog.tsx`
...
- [x] 7.1 `DeleteProjectDialog.test.tsx` (Testing Library via `@/test-utils`,
...
- [x] 7.2 Stories for OVR dogfooding:
...
- [x] 7.3 No E2E spec — the router integration + component tests cover the two
  seams.

## Verify

- [x] 8.1 `pnpm check-types`, `pnpm lint`, and the affected Vitest projects pass.
