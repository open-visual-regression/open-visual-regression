...
- [x] 1.1 Create `packages/ui/src/components/ui/badge.tsx`:
...
- [x] 1.2 Create `packages/ui/src/components/ui/key-hint.tsx`:
...
- [x] 1.3 Create `__stories__/badge.stories.tsx`: all 6 tones × filled=true/false in a grid (12 cells total)
- [x] 1.4 Create `__stories__/key-hint.stories.tsx`: J · K · A · R · ⌘K · ⇧Tab shown side by side
- [x] 1.5 Export `Badge`, `KeyHint` from `packages/ui/src/index.ts`; run `pnpm --filter @ovr/ui check-types` and `build-storybook` to confirm exits 0
