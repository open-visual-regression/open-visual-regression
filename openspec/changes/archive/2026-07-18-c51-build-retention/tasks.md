...
> Status: DONE (implemented outside OpenSpec). Shipped under `packages/builds/src/retention.ts`
> (not `packages/services/`), with `findExpiredPage`/`removeMany` on the builds repository and a
> transactional storage-outbox pattern. Baseline builds are protected by the cascade FK rolling back
> the delete rather than an explicit `existsForBuild` check — same guarantee. Repeatable `build:purge`
> job scheduled on worker startup (`schedulePurge`); covered by `retention.integration.test.ts`.
...
- [x] 1.1 `packages/db/src/repositories/builds.ts` — add:
...
- [x] 1.4 `packages/services/src/retention.ts`:
...
- [x] 1.5 `apps/worker/src/index.ts`: call `schedulePurge(redis)` on startup, alongside the existing `Worker` instances

- [x] 1.6 `apps/worker/src/handlers/purge.ts`:
...
- [x] 1.7 Unit tests for the purge handler (mock `purgeExpiredBuilds`): handler invokes the service function
