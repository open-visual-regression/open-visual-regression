...
- [x] 1.1 `apps/web/app/(authenticated)/projects/[projectId]/layout.tsx` (RSC):
...
- [x] 1.3 Add `retentionDays` column to `projects` table (`packages/db/src/schemas/schemas.ts`): `integer("retention_days").notNull().default(90)`. Run `drizzle-kit generate`; commit migration.

- [x] 1.4 `packages/api/src/contracts/projects.ts` — add contracts; update index:
...
- [x] 1.5 `apps/web/lib/router/projects.ts` — add handlers, each `.use(authenticatedMiddleware).use(adminMiddleware)` + `.actionable()`:
...
- [x] 1.6 Integration tests (`apps/web/lib/router/__tests__/projects.integration.test.ts`):
...
- [x] 1.7 `apps/web/app/(authenticated)/projects/[projectId]/settings/page.tsx` (RSC):
