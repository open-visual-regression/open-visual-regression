...
- [ ] 1.3 Create `apps/web/lib/auth-client.ts`:
...
- [ ] 1.4 Create `apps/web/app/api/auth/[...all]/route.ts`: mount Better Auth handler
  ```ts
  import { auth } from "@/lib/auth"
  export const { GET, POST } = auth.handler
  ```
- [ ] 1.5 Verify `GET http://localhost:3000/api/auth/session` returns `{ session: null }` when app is running
