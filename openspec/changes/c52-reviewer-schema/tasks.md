...
- [ ] 1.1 `packages/db/src/schemas/schemas.ts`: add `requiredReviewerCount: integer("required_reviewer_count").notNull().default(1)` to `projects`, alongside `diffThreshold`

- [ ] 1.2 `packages/db/src/schemas/builds.ts`:
  ```ts
...
- [ ] 1.3 `pnpm db:generate`; commit migration

- [ ] 1.4 `packages/db/src/repository/diffs.ts`:
...
- [ ] 1.5 `packages/db/src/repository/diffReviews.ts` (new):
...
- [ ] 1.6 `packages/db/src/client.ts`: register `diffReviews` repository on `dbClient`

- [ ] 1.7 `packages/db/src/repository/projects.ts`: add `requiredReviewerCount` to the `columns` allowlist in `getProject` and `listProjects`

- [ ] 1.8 Rewrite `packages/db/src/__tests__/diffs.integration.test.ts`:
...
- [ ] 1.9 New `packages/db/src/__tests__/diffReviews.integration.test.ts`:
...
- [ ] 1.10 `apps/worker/src/handlers/diff.ts`: update `failed` to call `dbClient.diffs.updateProcessingStatus(job.data.diffId, "error")` instead of the removed `updateStatus`

- [ ] 1.11 `apps/worker/src/handlers/__tests__/diff.integration.test.ts`: update assertions from `{ status: "error" }` to `{ processingStatus: "error" }` to match the renamed column
