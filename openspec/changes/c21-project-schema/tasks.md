...
- [ ] 1.2 `packages/db/src/repository/projects.ts` — add:
...
- [ ] 1.3 Add a `variants` table to `packages/db/src/schemas/schemas.ts`, following the existing `projects` table conventions (`uuidv7` default id, `pgTable`):
  ```ts
...
- [ ] 1.4 Create `packages/db/src/repository/variants.ts`:
  - `findByProject(projectId)` → variant[]
  - `addVariant(data)` → variant
  - `deleteVariant(id)` → void
  - `countByProject(projectId)` → number

- [ ] 1.5 Integration tests (`src/__tests__/integration/projects.test.ts`) using Testcontainers Postgres:
