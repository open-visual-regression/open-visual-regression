...
Gate: `drizzle-kit generate` adds a variant migration; existing integration tests (apps/web, against real Postgres) continue to pass.
...
- [x] 1.2 Add a `variants` table to `packages/db/src/schemas/schemas.ts`, following the existing `projects` table conventions (`uuidv7` default id, `pgTable`):
  ```ts
...
- [x] 1.3 Create `packages/db/src/repository/variants.ts`:
