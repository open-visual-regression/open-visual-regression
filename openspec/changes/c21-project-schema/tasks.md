...
- [x] 1.2 Create `packages/db/src/schema/projects.ts`:
  ```ts
  ...
  ```
- [x] 1.3 Run `drizzle-kit generate`; commit migration
- [x] 1.4 Create `packages/db/src/repositories/projects.ts`:
  ...
  - `delete(id)` → void
- [x] 1.5 Create `packages/db/src/repositories/variants.ts`:
  - `findByProject(projectId)` → variant[]
  - `create(data)` → variant
  - `delete(id)` → void
  - `countByProject(projectId)` → number
- [x] 1.6 Export all new repositories + schema from `packages/db/src/index.ts`
- [x] 1.7 Integration tests (`src/__tests__/integration/projects.test.ts`) using Testcontainers Postgres:
