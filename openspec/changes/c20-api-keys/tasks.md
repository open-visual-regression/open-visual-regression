...
## Phase 1 — API contracts + router (this PR)

- [x] 1.1 `packages/api/src/contracts/apiKeys.ts`: `create` contract (input: `{ name: string }`; output: `{ key: string }`), `list` contract (input: `{ limit, offset }`; output: `{ apiKeys: ApiKeyItem[], total: number }`), `revoke` contract (input: `{ keyId: string }`; output: void); update `contracts/index.ts`

- [x] 1.2 `apps/web/lib/router/os.ts`: add `adminMiddleware` derived builder — calls `auth.api.getSession`; throws `UNAUTHORIZED` if no session; throws `FORBIDDEN` if `session.user.role !== "admin"`; extends context with `{ session }`

- [x] 1.3 `apps/web/lib/router/apiKeys.ts`: `"use server"`;
...
- [x] 1.4 Integration tests: `apps/web/lib/router/__tests__/apiKeys.integration.test.ts`
...
## Phase 2 — UI (separate PR)

- [ ] 2.1 `apps/web/app/(authenticated)/settings/api-keys/page.tsx` (RSC):
...
- [ ] 2.2 `ApiKeyReveal` (`"use client"` component):
