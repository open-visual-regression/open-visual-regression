...
- [ ] 1 `apps/web/lib/router/apiKeys.ts` — `create`:
...
- [ ] 2 `packages/db/src/repository/apiKeys.ts` (new):
...
- [ ] 3 `packages/api/src/contracts/apiKeys.ts`:
...
- [ ] 4 `apps/web/lib/router/apiKeys.ts` — `list`: call `apiKeysRepo.findByProject(projectId, { limit, offset })` instead of `auth.api.listApiKeys` (which is scoped per-user, not per-project). `revoke` is unaffected — `auth.api.deleteApiKey` operates on a key id directly.

- [ ] 5 Integration tests: `apps/web/lib/router/__tests__/apiKeys.integration.test.ts`
