...
- [ ] 1.1 `packages/services/src/snapshots.ts`: rewrite `diffSnapshot(snapshotId, diffId)`:
...
- [ ] 1.2 `packages/services/src/__tests__/fixtures.ts`: add a `featureBuild` fixture alongside `build`, identical except `branch: "feature/test"`, for tests that need a non-main-branch build

- [ ] 1.3 `packages/services/src/__tests__/snapshots.integration.test.ts`:
