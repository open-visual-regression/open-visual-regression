...
export const find = (projectId: string, captureConfigurationId: string, storyId: string) =>
...
export const upsert = async (values: typeof baselines.$inferInsert) => {
  const [baseline] = await db
    ...
    .values(values)
    .onConflictDoUpdate({
      target: [baselines.projectId, baselines.captureConfigurationId, baselines.storyId],
      set: {
